...
\centering
  ...
  \end{lstlisting}
\caption[Evento]{El evento ``onmousemove'', que se dispara en el momento en el que el ratón se desplaza es vinculado a la función \texttt{manejadorMouseMove} en una página web.}
\end{figure}
...
\centering
  ...
  \end{lstlisting}
\caption[Manejador]{Implementación de la funcionalidad a realizar cuando el ratón es desplazado.}
\end{figure}
...
Un tercer enfoque es desarrollo de aplicaciones dirigidas por eventos (\textit{event-driven}) en un único hilo (\textit{single-threaded application}). Para conseguir paralelizar las diferentes tareas se deben ejecutar de forma no bloqueante, cediendo la CPU a otra tarea en caso de que se deba esperar a que una tarea sea realizada (generalmente, operaciones de entrada-salida, que presentan un gran tiempo de espera sin consumo de CPU, como la descarga de información en red o el acceso al almacenamiento secundario).
...
El algoritmo del abusón\citationneeded posibilita la elección de un coordinador en función del cumplimiento de un criterio cuantitativo dado, siendo elegido coordinador aquel que mejor satisfaga dicho criterio. La secuencia de pasos es la siguiente:
